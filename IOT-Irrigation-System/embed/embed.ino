...
void getTimeListFromAPI() {
  HTTPClient http;
  String url = "http://192.168.109.66:3000/time"; // Địa chỉ API
...
void updatePumpStatus(bool isPumpOn) {
  HTTPClient http;
  String url = "http://192.168.109.66:3000/auto/create"; 
...
String getPumpStatus() {
  HTTPClient http;
  String url = "http://192.168.109.66:3000/status/detail/1"; // Địa chỉ API
  if (http.begin(url)) {
    int httpResponseCode = http.GET();
    if (httpResponseCode == HTTP_CODE_OK) {
      String payload = http.getString();
